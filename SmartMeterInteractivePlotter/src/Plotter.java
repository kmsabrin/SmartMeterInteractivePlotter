...
public class Plotter extends javax.swing.JFrame {
	...
	public Plotter(String title) {
		...
		meterData = dbTunnel.getData(buildingMap.get(buildingName), startTime,
				endTime, startMonth, endMonth, startYear, endYear);
		...
		((JLabel) chartDataComboBox.getRenderer())
		...
		((JLabel) startMonthComboBox.getRenderer())
				.setHorizontalAlignment(SwingConstants.CENTER);
		...
		((JLabel) startYearComboBox.getRenderer())
				.setHorizontalAlignment(SwingConstants.CENTER);
		...
		((JLabel) endMonthComboBox.getRenderer())
				.setHorizontalAlignment(SwingConstants.CENTER);
		...
		((JLabel) endYearComboBox.getRenderer())
				.setHorizontalAlignment(SwingConstants.CENTER);
		...
		chartDataComboBox.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (chartDataType.equals("Q-V")) {
					xAxis = "V";
					yAxis = "Q";
				}
				if (chartDataType.equals("P-Q")) {
					xAxis = "P";
					yAxis = "Q";
				}
	...
	private void createDataset() {
		...
		for (int i = 0; i < meterData.P.size(); ++i) {
			if (meterData.P.get(i) < -9.0 || meterData.Q.get(i) < -9.0)
				continue;
	...
	private void createChart(XYDataset dataset) {
		// create the chart...
		chart = ChartFactory.createScatterPlot("Smart Grid Chart", // chart
																	// title
				xAxis, // x axis label
				yAxis, // y axis label
				dataset, // data
