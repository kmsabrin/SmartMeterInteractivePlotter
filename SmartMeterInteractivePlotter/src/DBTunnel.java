...
public class DBTunnel {
	...
	MeterData getData(String buildingID, String startTime, String endTime,
			String startMonth, String endMonth, String startYear, String endYear) {
		Connection c = null;
		Statement stmt = null;
		...
		try {
			...
			// System.out.println(sTime + "--" + eTime + "--" + sMonth + "--" +
			// eMonth + "--" + sYear + "--" + eYear);

			stmt = c.createStatement();
			String sql = null;
			if (eYear > sYear) {
				sql = "SELECT P, Q, V FROM meters WHERE " + "BLDG_ID == '"
						+ buildingID + "' AND " + "time >= " + sTime + " AND "
						+ "time <= " + eTime + " AND " + "(( month >= "
						+ sMonth + " AND " + "year == " + sYear + ") OR"
						+ " (month <= " + eMonth + " AND " + "year == " + eYear
						+ ") OR" + " (year > " + sYear + " AND " + "year < "
						+ eYear + "));";
			} else {
				sql = "SELECT P, Q, V FROM meters WHERE " + "BLDG_ID == '"
						+ buildingID + "' AND " + "time >= " + sTime + " AND "
						+ "time <= " + eTime + " AND " + "month >= " + sMonth
						+ " AND " + "month <= " + eMonth + " AND " + "year == "
						+ eYear + ";";
			}
